...
public class StudentImportHelper {
    ...
    private static void parseExcel(InputStream is, PasswordEncoder encoder, ImportResult result) throws Exception {
        ...
        for (Row row : sheet) {
            ...
            // Columns: Register Number, Name, Department, Year, Email
            ...
            if (isEmpty(regNo) || isEmpty(name) || isEmpty(dept) || isEmpty(year) || isEmpty(email)) {
    ...
    private static void parseCsv(InputStream is, PasswordEncoder encoder, ImportResult result) throws Exception {
        try (BufferedReader reader = new BufferedReader(new InputStreamReader(is, StandardCharsets.UTF_8))) {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                if (tokens.length < 5) {
                    result.logs.add("Row " + rowNumber + ": Missing columns. Required format: RegisterNo, Name, Department, Year, Email.");
                    result.errorCount++;
                    continue;
                }
